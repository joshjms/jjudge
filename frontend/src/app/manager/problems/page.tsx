		...
		<div className="space-y-8">
			...
			{!hasToken ? (
				<p className="border border-destructive/50 bg-destructive/10 px-4 py-3 text-xs font-mono text-destructive">
					Sign in to access manager tools.
				</p>
			) : null}
			...
			<div className="grid gap-10 lg:grid-cols-[1.1fr,0.9fr]">
						...
						<div className="flex flex-col divide-y divide-border/40">
							{problems.map((problem) => (
								<div key={problem.id} className="group flex items-center gap-3 py-3 px-1 hover:bg-muted/30 transition-colors">
									...
									<button
										type="button"
										className="border border-border/60 px-2.5 py-1 text-[10px] font-mono tracking-widest text-muted-foreground hover:border-primary/60 hover:text-primary transition-colors shrink-0"
				...
				<section className="space-y-6 border border-border/60 bg-background/70 p-6">
					...
					{editingId === null && (
						<p className="text-[10px] font-mono text-amber-700 border border-amber-500/50 bg-amber-500/10 px-3 py-2 tracking-widest">
							New problems will be set to PENDING until approved by an admin.
						</p>
					)}
					<form className="space-y-5" onSubmit={handleSubmit}>
						...
						<div className="space-y-5">
							...
							{groups.map((group, groupIndex) => (
								<div key={group.id} className="border border-border/60 p-4 space-y-4">

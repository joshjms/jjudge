...
func (r *ContestRepository) ListContests(ctx context.Context, offset, limit int) ([]types.Contest, int, error) {
	const countQuery = `SELECT COUNT(*) FROM contests`
	var total int
	...
	const query = `
		SELECT id, title, description, start_time, end_time,
		       scoring_type, visibility, owner_id, created_at, updated_at
		FROM contests
...
func (r *ContestRepository) CreateContest(ctx context.Context, c types.Contest) (types.Contest, error) {
	...
	const query = `
		INSERT INTO contests (title, description, start_time, end_time, scoring_type, visibility, owner_id, created_at, updated_at)
...
func (r *ContestRepository) UpdateContest(ctx context.Context, c types.Contest) (types.Contest, error) {
	...
	const query = `
		...
		SET title = $1, description = $2, start_time = $3, end_time = $4,
		    scoring_type = $5, visibility = $6, updated_at = $7
		WHERE id = $8`
	result, err := r.db.ExecContext(ctx, query,
		c.Title, c.Description, c.StartTime, c.EndTime,
		c.ScoringType, c.Visibility, c.UpdatedAt, c.ID,
	)
	if err != nil {
		return types.Contest{}, err
	}

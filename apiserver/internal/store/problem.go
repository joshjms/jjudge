...
func (r *ProblemRepository) List(ctx context.Context, offset, limit int, callerID int, isAdmin bool) ([]types.Problem, int, error) {
	if offset < 0 {
		offset = 0
	}
	if limit < 1 {
	...
	var countArgs []any
	if isAdmin {
		countQuery = `SELECT COUNT(1) FROM problems`
	} else if callerID > 0 {
		countQuery = `SELECT COUNT(1) FROM problems WHERE (visibility = 'public' AND approval_status = 'approved') OR creator_id = $1`
		countArgs = []any{callerID}
	} else {
		countQuery = `SELECT COUNT(1) FROM problems WHERE visibility = 'public' AND approval_status = 'approved'`
	}
	if err := r.db.QueryRowContext(ctx, countQuery, countArgs...).Scan(&total); err != nil {
		return nil, 0, err
	}

	const cols = `SELECT id, title, description, difficulty, time_limit, memory_limit, tags, creator_id, approval_status, visibility, created_at, updated_at FROM problems`
	var listQuery string
	var listArgs []any
	if isAdmin {
		listQuery = cols + ` ORDER BY id OFFSET $1 LIMIT $2`
		listArgs = []any{offset, limit}
	} else if callerID > 0 {
		listQuery = cols + ` WHERE (visibility = 'public' AND approval_status = 'approved') OR creator_id = $1 ORDER BY id OFFSET $2 LIMIT $3`
		listArgs = []any{callerID, offset, limit}
	} else {
		listQuery = cols + ` WHERE visibility = 'public' AND approval_status = 'approved' ORDER BY id OFFSET $1 LIMIT $2`
		listArgs = []any{offset, limit}
	}
	...
	for rows.Next() {
		...
		if creatorID.Valid {
			problem.CreatorID = int(creatorID.Int64)
		}
...
func (r *ProblemRepository) ListPending(ctx context.Context, offset, limit int) ([]types.Problem, int, error) {
	if offset < 0 {
		offset = 0
	}
	if limit < 1 {
	...
	const countQuery = `SELECT COUNT(1) FROM problems WHERE approval_status = 'pending'`
	var total int
	...
	const listQuery = `
		SELECT id, title, description, difficulty, time_limit, memory_limit, tags, creator_id, approval_status, visibility, created_at, updated_at
		FROM problems
...
func (r *ProblemRepository) ListByCreator(ctx context.Context, creatorID, offset, limit int) ([]types.Problem, int, error) {
	if offset < 0 {
		offset = 0
	}
	if limit < 1 {
	...
	const countQuery = `SELECT COUNT(1) FROM problems WHERE creator_id = $1`
	var total int
	...
	const listQuery = `
		...
		ORDER BY id
		OFFSET $2 LIMIT $3`
	rows, err := r.db.QueryContext(ctx, listQuery, creatorID, offset, limit)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if cID.Valid {
			problem.CreatorID = int(cID.Int64)
		}
		_ = json.Unmarshal(tagsJSON, &problem.Tags)
		problems = append(problems, problem)
	}
...
func (r *ProblemRepository) Create(ctx context.Context, problem types.Problem) (types.Problem, error) {
	...
	const query = `
		INSERT INTO problems (title, description, difficulty, time_limit, memory_limit, tags, creator_id, approval_status, visibility, created_at, updated_at)
...
func (r *ProblemRepository) Approve(ctx context.Context, id int) error {
	const query = `UPDATE problems SET approval_status = 'approved', updated_at = $1 WHERE id = $2`
...
func (r *ProblemRepository) Reject(ctx context.Context, id int) error {
	const query = `UPDATE problems SET approval_status = 'rejected', updated_at = $1 WHERE id = $2`
	result, err := r.db.ExecContext(ctx, query, time.Now(), id)
	if err != nil {
		return err
	}
